...
import { Zap, Briefcase, GitPullRequest, ArrowUpRight } from 'lucide-react';
    ...
    <div className="space-y-6">
        ...
        {INSIGHTS.map((insight, idx) => (
          <div key={idx} className="bg-slate-900 border border-slate-800 p-5 rounded-xl hover:bg-slate-800/50 transition-colors">
            <div className="flex items-start justify-between mb-2">
                <span className="text-xs font-mono text-slate-500 uppercase">{insight.category}</span>
                <ArrowUpRight size={16} className="text-slate-600" />
            </div>
            <h3 className="text-slate-100 font-bold mb-2">{insight.title}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data}>
                ...
                />
                <Area type="monotone" dataKey="capability" stroke="#8b5cf6" fillOpacity={1} fill="url(#colorCap)" name="Capability Capital" />
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl flex flex-col justify-between">
           <div>
              <h3 className="text-lg font-bold text-slate-200 mb-4">Core Shifts</h3>
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                   <div className="p-2 bg-red-500/10 rounded-lg text-red-400"><Briefcase size={20}/></div>
                   <div>
                       <div className="text-xs text-slate-500 uppercase">Asset Class</div>
                       <div className="text-sm font-medium text-slate-200">Firm → Capability</div>
                   </div>
                </div>
                <div className="flex items-center gap-4">
                   <div className="p-2 bg-blue-500/10 rounded-lg text-blue-400"><GitPullRequest size={20}/></div>
                   <div>
           ...
           <div className="mt-8 pt-6 border-t border-slate-800">
               <div className="text-xs font-mono text-slate-500 mb-2">FUTURE STATE</div>
